...
def climate_decision(task,w):
    ...
    if task.get("prefer_after_rain",False) and rain > 2:
        return "ESPERAR POST-LLUVIA", f"viene lluvia ({rain:.1f} mm/72 h); mejor después del frente si corresponde"
...
def candidate_tasks(kb,today):
    ...
    for sp in active_species(kb):
        for task in sp.get("tasks",[]):
            ...
            if -1 <= (d-today).days <= HORIZON_DAYS:
                rows.append({"date":d,"species":sp["name"],"latin":sp.get("latin",""),**task})
    return sorted(rows,key=lambda r:(PRIORITY.get(r.get("priority","media"),1),r["date"],r["species"]))[:MAX_EVENTS]
...
def make_ics(tasks,w,today):
    ...
    lines=[
        "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Podas Pumalal ICS Bot//ES",
        "CALSCALE:GREGORIAN","METHOD:PUBLISH",
        "X-WR-CALNAME:Podas Pumalal Dinámico","X-WR-TIMEZONE:America/Santiago",
        "REFRESH-INTERVAL;VALUE=DURATION:PT12H","X-PUBLISHED-TTL:PT12H"
    ]
    desc=(f"Generado: {generated.strftime('%d-%m-%Y %H:%M')}\\n"
    ...
    start=datetime.combine(today,time(7,45),tzinfo=tz); end=start+timedelta(minutes=15)
    add_event(lines,f"pumalal-resumen-{today.isoformat()}@podas",start,end,"🌿 Resumen climático Podas Pumalal",desc)

    for task in tasks:
        status,reason=climate_decision(task,w)
        event_day=task["date"] if status.startswith("HACER") else today
        start=datetime.combine(event_day,time(8,30),tzinfo=tz); end=start+timedelta(minutes=30)
        icon="🌿" if status.startswith("HACER") else "⚠️"
        summary=f"{icon} {status}: {task.get('category','tarea')} · {task['species']}"
        desc=(f"Especie: {task['species']} ({task.get('latin','')})\\n"
...
def main():
    ...
    with open("podas_pumalal_dinamico.ics","w",encoding="utf-8") as f:
